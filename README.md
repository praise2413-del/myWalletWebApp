...
See [docs/ui-design.md](docs/ui-design.md) for the design system, [docs/architecture.md](docs/architecture.md) for project structure, [docs/database.md](docs/database.md) for the schema, [docs/insights.md](docs/insights.md) for the insights engine, [docs/reports.md](docs/reports.md) for reporting/period logic, [docs/security.md](docs/security.md) for the threat model, [docs/testing.md](docs/testing.md) for how the app is verified, and [docs/deployment.md](docs/deployment.md) for going live on Vercel.
...
- ✅ **Phase 8 — Categories**: full CRUD — create custom categories with a name and icon picker, edit a category's name/icon (its income/expense type is locked after creation to keep past transactions consistent), delete custom categories (default categories can't be deleted, and a category still holding transactions is blocked with a friendly error instead of a raw FK-constraint message). Category icons resolve by a stored icon slug rather than by name, so renaming a category never changes its icon.
- ✅ **Phase 9 — Settings data & privacy**: profile/preferences/security tabs (from Phase 3) plus Export Data (downloads every transaction and savings/investment allocation as a CSV) and Delete Account (permanently deletes the account and all its data — requires typing "DELETE" to confirm, backed by a `SECURITY DEFINER` database function so it can only ever delete the caller's own account).
- ✅ **Phase 10 — Quality**: accessibility (every modal/slide-over/mobile-drawer now traps focus, moves it in on open, and restores it on close — `src/hooks/useFocusTrap.ts`), a real security gap closed at the database layer (a category's `type`/`is_default` could previously be changed via a direct API call even though the UI never allowed it — see `docs/security.md`), a bundle-size fix (vendor code split into its own cacheable chunks, cutting the main app bundle from ~511kB to ~120kB), and 27 new unit tests for previously-uncovered utils (85 total). See `docs/security.md` and `docs/testing.md`.
- ✅ **Phase 11 — Deployment**: repo is deploy-ready for Vercel — `vercel.json` (type-checked build command, SPA routing rewrite, security headers including a CSP scoped to exactly what the app loads, long-lived caching for fingerprinted assets), `package.json` `engines.node`. HTTPS is automatic on Vercel. Connecting the actual Vercel account/domain and updating Supabase's Auth Site URL/Redirect URLs to match are steps only the project owner can do — see `docs/deployment.md` for the exact checklist.
