...
export function PurchaseDetailModal({
  ...
  const status = purchase ? statusFor(purchase) : null;
  const remaining = purchase ? purchase.total - purchase.amountPaid : 0;

  return (
    <Modal
      open={Boolean(purchase)}
      onClose={onClose}
      title={purchase?.billNumber ? `Bill ${purchase.billNumber}` : "Purchase Bill"}
      description={purchase ? `${purchase.supplierName ?? "Cash purchase"} · ${formatDate(purchase.billDate)}` : undefined}
      size="md"
    >
      {purchase && status && (
        <div className="space-y-4">
            ...
            <table className="w-full text-sm">
              ...
              <tfoot>
                <tr className="border-t border-border bg-background font-semibold text-text-primary">
                  ...
                  <td className="px-3 py-2 text-right">{formatCurrency(purchase.total, currency)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
              ...
              <div className="space-y-1.5">
                {purchase.payments.map((p) => (
                  <div key={p.id} className="flex items-center justify-between rounded-lg bg-background px-3 py-2 text-sm">
                    ...
                    <span className="font-medium text-expense-600">{formatCurrency(p.amount, currency)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {purchase.notes && <p className="text-sm text-text-secondary">{purchase.notes}</p>}
        </div>
      )}
    </Modal>
  );
